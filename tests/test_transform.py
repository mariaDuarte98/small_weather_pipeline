...
class TestComputeRollingAvg:
    ...
    def test_adds_rolling_column(self):
        ...
        assert "tempmax_rolling_7d" in result.columns
    ...
    def test_rolling_avg_matches_single_row(self):
        ...
        assert result["tempmax_rolling_7d"].iloc[0] == pytest.approx(10.0)
...
class TestTransformWeather:
    ...
    def test_returns_empty_on_none(self):
        result = transform_weather(None)
        assert result.empty

    def test_returns_empty_on_empty_df(self):
        result = transform_weather(pd.DataFrame())
        assert result.empty
